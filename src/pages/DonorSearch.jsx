...
const DonorSearch = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await searchNearestDonors({ address, bloodType });
    if (response.success) {
    ...
    <option value="receiver">Người nhận</option>
      ...
      {donors.length > 0 ? (
        <div className="donor-list">
          <h3>Kết quả ({donors.length} nhà tài trợ):</h3>
          {donors.map((donor) => (
        ...
        <p>Chưa có nhà tài trợ nào được tìm thấy.</p>
      )}
