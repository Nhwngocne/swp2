// ✅ Giao diện Quản lý Tin tức đẹp như mẫu ảnh: Quản lý + Tạo bài viết (2 cột)

import React, { useState, useEffect } from "react";
import { useEvents } from "../../services/EventContext";
import { CKEditor } from "@ckeditor/ckeditor5-react";
import ClassicEditor from "@ckeditor/ckeditor5-build-classic";
import "../../assets/css/components/admin/NewsManager.css"; // Ensure you have this CSS file for styling
const NewsManager = () => {
  const {
    blogs,
    fetchBlogs,
  ...
  const handleSubmit = async (e) => {
    ...
    setFormError(null);
    let imageUrl = "";
    if (formData.image) imageUrl = await handleUpload(formData.image);

    const blog = { ...formData, image: imageUrl };
    const res = editing ? await updateBlog(editing.id, blog) : await createBlog(blog);
  ...
  return (
    <div className="container py-4">
      <div className="d-flex justify-content-between mb-3">
        ...
        <button className="btn btn-danger" onClick={() => setShowForm(true)}>➕ Tạo Mới</button>
      </div>
          ...
          <table className="table table-bordered align-middle">
            ...
            <tbody>
              {blogs.map((item, index) => (
                <tr key={item.id}>
                  <td>{index + 1}</td>
                  <td><img src={item.image} alt="thumb" width="50" /></td>
                  ...
                  <td className="d-flex gap-1">
                    ...
                    <button className="btn btn-sm btn-danger" onClick={() => deleteBlog(item.id)}>❌</button>
                  </td>
                </tr>
              ))}
            </tbody>
