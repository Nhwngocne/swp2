  ...
  try {
    ...
    if (historyData.id) {
      // Nếu có id của history => cập nhật lịch sử hiến máu
      historyResponse = await updateDonationHistory(historyData.id, historyData);
    } else {
      // Nếu chưa có id thì tạo mới lịch sử hiến máu
      historyResponse = await createDonationHistory(historyData);
    }

    if (!historyResponse.success) {
      alert(`Lỗi khi lưu lịch sử hiến máu: ${historyResponse.error}`);
      return;
    }
